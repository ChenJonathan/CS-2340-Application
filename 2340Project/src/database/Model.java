...
public class Model {
    ...
    public final boolean addReport(final UserReport report) {
        try {
            String body = "&locationName="
                    ...
                    + report.getLongitude()
                    + "&description="
                    + URLEncoder.encode(report.getDescription().get(), "UTF-8")
                    + "&timestamp="
                    + URLEncoder.encode(report.getTimestamp().get(), "UTF-8")
                    + "&user="
                    + URLEncoder.encode(report.getAuthor().get(), "UTF-8")
                    + "&waterType="
    ...
    /**
     * Retrieves all reports from the
     * database within a specified
     ...
     *               from the points to find reports
     * @return all the reports in the
     *              database that are
     *              close to a specific location
     */
    public final ObservableList<Report> getReportsByLocation(final double longitude,
                                                             final double latitude,
                                                             final double radius) {
        ObservableList<Report> reports = FXCollections.observableArrayList();
        try {
            HttpResponse<JsonNode> response = Unirest.get("https://chenjonathan"
                    + "-cs-2340-api.herokuapp.com/report/location?"
